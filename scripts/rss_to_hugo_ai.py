...
def _parse_openai_error(response):
    try:
    ...
    if isinstance(error, dict):
        return error.get("type") or error.get("code"), error.get("message")
    return None, None
...
def _handle_rate_limit(response, context):
    if response.status_code != 429:
        return
    error_type, error_message = _parse_openai_error(response)

    fatal_quota = False
    if error_type and "insufficient_quota" in error_type:
        fatal_quota = True
    elif error_message:
        lowered = error_message.lower()
        fatal_quota = "insufficient quota" in lowered or "exceeded your current quota" in lowered or "billing hard limit" in lowered

    if fatal_quota:
        raise SystemExit(
            "OpenAI API quota appears to be exhausted. "
            f"Context: {context}. Message: {error_message or 'insufficient quota'}"
        )
    ...
    extra_info = f" ({error_message})" if error_message else ""
    print(
        f"OpenAI rate limit hit during {context}. Waiting {int(wait_for)}s before retrying...{extra_info}",
        flush=True,
    )
    time.sleep(wait_for)
